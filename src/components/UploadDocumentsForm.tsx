...
export function UploadDocumentsForm() {
  ...
  const [document, setDocument] = useState("Hello my name is Dhruv");
  ...
  return (
    <form onSubmit={ingest} className="flex w-full mb-4 text-black">
      <MyDropzone setDocument={setDocument}/>
    <button type="submit" className="shrink-0 px-8 py-4 bg-sky-600 rounded w-28">
      ...
      <span className={isLoading ? "hidden text-white" : "text-white"}>Upload</span>
    </button>
  </form>
  );
}